...
void ErrFunc(char* er, char* mod, char* ver,int line, char* file)
{
	short ct=0;
...
{
	//TPTR t = malloc(sizeof(struct Txt));
...
ITEM *CreateItem(char *name, int ad, int no)
{
	...
	O_ADJECTIVE(a) = (short)ad;		/* Adjective as asked   */
	O_NOUN(a) = (short)no;			/* Noun as was asked    */
...
void LineDump(void)
{
	...
	fprintf(stderr, "$ME is %"PRIuPTR" %s\n", (uintptr_t)Me(), Valt(Me()));
	fprintf(stderr, "$1  is %"PRIuPTR" %s\n", (uintptr_t)Item1, Valt(Item1));
	fprintf(stderr, "$2  is %"PRIuPTR" %s\n", (uintptr_t)Item1, Valt(Item2));
	fprintf(stderr, "$AC is %"PRIuPTR" %s\n", (uintptr_t)Actor(), Valt(Actor()));
	if(CurrentLine)
	{
		FPCurrentLine();
	}
...
int ICheck(register ITEM *item, int line, char *file)
{
	...
	fprintf(stderr,"Pointer=%"PRIuPTR"!\n", (uintptr_t)item);
	LineDump();
//	ErrFunc("Invalid ITEM *","<VALIDATOR>","1.00",line,file);
	fprintf(stderr, "Invalid item: %s:%d", file, line);
	return 0;
}
