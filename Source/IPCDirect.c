...
static unsigned int MainFD,AltFD,BsxFD;
...
int FindUserFD(unsigned int fd)
{
	int ct = 0;
...
int ReadBlock(int u, COMTEXT *pkt)
{
	...
	while ((l = recv(UserList[u].us_Port->po_fd, &c, 1, 0)) > 0)
	{
...
#else
			recv(UserList[u].us_Port->po_fd, &c, 1, 0);
#endif
		...
		{
			c = '\n';
			InputPos[u] = (short)strlen(UserInput[u]);
		}
		if (c == 8 || c == 127)
		{
	...
	if ((l == -1 && error != WSAEWOULDBLOCK) /*|| gotany == 0*/)
#endif
	{
		/* Network fault.. shove it */
		UserList[u].us_Port->po_Flags |= FL_FAULT;
		FD_CLR((u_int)UserList[u].us_Port->po_fd, &ReadMask);
		FD_CLR((u_int)UserList[u].us_Port->po_fd, &WriteMask);
	}
	return(0);
}
	...
	switch(block->pa_Type)
	{
		...
		case PACKET_SETPROMPT:
			...
			else
			{
				...
				WriteSocket(u, block->pa_Data, (int)strlen(block->pa_Data));
				WriteSocket(u,"\003",1);
			}
			break;
			...
			switch(UserState[u])
			{
				case 2:
					if(!isalpha(*Prompts[u]))
						break;
					WriteSocketText(u, Prompts[u], (int)strlen(Prompts[u]));
					WriteSocketText(u,"\r\n",2);
					break;
				case 0:
					WriteSocketText(u, Prompts[u], (int)strlen(Prompts[u]));
...
{
	struct timeval tvl;
	static time_t magic_time_cookie = 0;
	time_t t;
...
#else
	memmove(port->po_Silo, port->po_Silo + sz, port->po_SiloPtr);
#endif
...
int MakeConnection(unsigned int fd, COMTEXT *p)
{
	struct sockaddr_in netaddr;
	int lv=sizeof(netaddr);
#ifdef _WIN32
	SOCKET ofd = fd;
#else
	int ofd=fd;
...
#endif
	fd = (int)accept(fd, (struct sockaddr *)&netaddr, &lv);
#if !defined AMIGA && !defined _WIN32
	fcntl(fd,F_SETFL,0);
#endif

	p->pa_Type=PACKET_LOGINREQUEST;
	p->pa_Sender=(short)fd;
	...
	sprintf(p->pa_Data, "%s%s$%d$*", "Internet:", NetName(netaddr.sin_addr.s_addr), (int)(fd + (ofd == AltFD ? 256 : 0) + (ofd == BsxFD ? 512 : 0)));
