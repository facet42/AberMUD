...
void Act_NewText(void)
{
	...
	if(UserOf(Me())>=0)
	{
		struct ParserContext *p=GetContext((short)UserOf(Me()));
		p->pa_It[0]=-1;
		p->pa_It[1]=-1;
		p->pa_Them[0]=-1;
		p->pa_Them[1]=-1;
...
{
	ITEM *i=ArgItem();
	short cm= (short)ArgNum();
	short num= (short)ArgNum();
...
void Act_Dup(void)
{
	...
	short f= (short)ArgNum();	/* 1 = true dup  */
	short g= (short)ArgNum();	/* $(x)=new item */
	ITEM *n;
	n=Clone_Item(i,f);
	if(g==1)
		Item1=n;
...
void Act_Means(void)
{
	...
	if(ArgNum())
	{
		short Noun3= (short)Noun1;	/* Selected SWAPword */
		Noun1=Noun2;
		Noun2=Noun3;
	}
}
...
void Act_Parse(void)
{
	...
	char *ow=WordPtr;
	int v;
	short oa1 = (short)Adj1, oa2 = (short)Adj2, on1 = (short)Noun1, on2 = (short)Noun2, ov = (short)Verb, op = (short)Prep;
...
{
	short w= (short)ArgWord();
	short u= (short)UserOf(Me());
	if(u<0)
		return;
...
{
	short ad,no;
	short d= (short)ArgNum();
	ad=(d==1)? (short)Adj1: (short)Adj2;
	no=(d==1)? (short)Noun1: (short)Noun2;
	d= (short)ArgNum();
	if(d==1)
	{
...
void Act_Field(void)
{
	...
	{
		SendUser(-2, "");
		SendNPacket(UserList[u].us_Port, PACKET_SETFIELD, (short)ArgNum(), 0, 0, 0);
	}
	else
...
void Act_Unveil(void)
{
	...
	sscanf(TextOf(TxtArg), "%x,%x,%x,%x,%x,%x,%x,%x", &a, &b, &c, &d, &e, &f, &g, &h);

	if ((unsigned int)rand() % 256 != a)
	{
		Log("Failed UNVEIL");
		return;
	}

	if((unsigned int)rand()%256!=b)
	{
		Log("Failed UNVEIL");
		return;
	}

	if((unsigned int)rand()%256!=c)
	{
		Log("Failed UNVEIL");
		return;
	}

	if((unsigned int)rand()%256!=d)
	{
		Log("Failed UNVEIL");
		return;
	}

	if((unsigned int)rand()%256!=e)
	{
		Log("Failed UNVEIL");
		return;
	}

	if((unsigned int)rand()%256!=f)
	{
		Log("Failed UNVEIL");
		return;
	}

	if((unsigned int)rand()%256!=g)
	{
		Log("Failed UNVEIL");
		return;
	}

	if((unsigned int)rand()%256!=h)
	{
		Log("Failed UNVEIL");
		return;
	}
