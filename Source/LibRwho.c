...
#if !defined NO_HUGE_RESOLVER_CODE && !defined _WIN32
...
int rwhocli_setup(char *server, char *serverpw, char *myname, char *comment)
{
...
#else
		memmove(&addr.sin_addr, &f, sizeof(f));
#endif
	}
	...
	if((dgramfd = (int)socket(AF_INET,SOCK_DGRAM,0)) < 0)
		return(1);
	...
	sendto(dgramfd, pbuf, (int)strlen(pbuf), 0, (struct sockaddr *)&addr, (int)sizeof(addr));
	return(0);
}
