	...
	while (ct < MAXUSER)
	{
		if (UserList[ct].us_Port != NULL)
		{
			...
			if (LineFault[ct] || UserList[ct].us_Port->po_Flags & FL_FAULT)
			{
				...
				{
					int flag = GetUserFlag(UserList[ct].us_Item, 2);
					if (flag != 0)
					{
						ct++;
						continue;
...
int SendTPacket(PORT *us, short class, char *data)
{
	...
	int result = SendBlock(us, &a, (int)(strlen(data) + 2 * (sizeof(short)) + 1));
	return result;
}
...
oops:
		...
		*msg++=0;
		...
		UserList[ct].us_Port=port;
		time((time_t*)&(UserList[ct].us_Login));
		if(*ubf!='*')
 ...
 */
		...
		SendNPacket(port, PACKET_LOGINACCEPT, 0, (short)ct, 0, 0); /* login ok */
