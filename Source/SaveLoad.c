...
static unsigned short LoadShort(FILE* x)
{
	unsigned char a[3];
...
TPTR s;
{
	...
	register long v=(long)strlen(str);
	if(s->te_Users==1)
		v|=65536;
	SaveLong(file,v);
...
static TPTR LoadString(FILE* file)
{
	register char *a;
...
BSXImage *i;
{
	int v=(int)strlen(i->bsx_Identifier);
	SaveLong(file,v);
 ...
 */
	time((time_t*)&(header[2]));	/* Save time */
...
register ITEM *i;
{
	...
	if(i->it_Superclass)
	{
		...
		SaveShort(file,0);
	SaveLong(file,(unsigned long)(uintptr_t)i->it_Properties);
	n=i->it_Properties;
	if(n==0)
		return;
	while(n)
