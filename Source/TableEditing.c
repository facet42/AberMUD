...
/* Show New Line - Note G alone shows line */
	...
	UserList[u].us_UserInfo=(short)line;
}
...
char *arg;
{
	...
	LINE *l;
	strcpy(vs,"ANY");
	if ((nps = (short)sscanf(arg, "%29s %29s %29s %d %d", vs, n1s, n2s, &tl, &bl)) < 1)
	{
...
char *arg;
{
	UserList[u].us_UserInfo = (short)CountEntries((TABLE *)UserList[u].us_UserPtr) - 1;
	Tbl_Goto(u,"");
}
...
char *arg;
{
	...
	n->li_Noun1=-1;
	n->li_Noun2=-1;
	UserList[u].us_UserInfo = (short)(line + ia);
	Tbl_Edit(u,"");
}
