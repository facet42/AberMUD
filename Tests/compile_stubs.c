/*
 * Stub/support file for unit-testing the table compiler (CompileTable.c)
 * together with the real Parser.c (needed for FindInList/AddWord/WordList
 * so compiled verb/noun/adj tokens resolve against real word-list
 * entries) and the same VM files test_vm.c already covers
 * (CondCode.c/ActionCode.c/TableDriver.c), so a compiled line can be
 * executed and checked, not just inspected as bytecode.
 *
 * This is a near-duplicate of vm_stubs.c (see its comments for why each
 * real-vs-stub choice was made) MINUS the handful of symbols that
 * Parser.c and CompileTable.c themselves now supply for real --
 * WordPtr, GetContext, SetItData, GetRestOfInput, GetOrd, GetPrep,
 * GetThing, GetVerb (from Parser.c), and PairArg (from CompileTable.c)
 * -- since linking both the real source file and a stub of any of
 * those would be a duplicate definition. Kept as its own file rather
 * than sharing vm_stubs.c because C has no clean way to "reuse a stub
 * file but omit one symbol" at the file level.
 *
 * New beyond vm_stubs.c: AllocComment/FreeComment (table comment text),
 * GetClassName/GetFlagByName/GetFlagName/WhichClass (name-registry
 * lookups for EncodeClass/EncodeFlag's "@name" form -- not exercised
 * here, trivial), and IsUnique (Decompress()'s "does this item need a
 * #N disambiguator" check). FindMaster is upgraded from vm_stubs.c's
 * always-NULL stub to always return one fixed fake item, so a test can
 * compile a literal (non-$-sentinel) item reference.
 */
